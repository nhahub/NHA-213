...
router.get("/", authMiddleware, roleAuth("admin"), async (req, res) => {
  ...
  .populate("userId", "name email") // include only name and email
  .populate("deliveryAgentId", "name email"); // include name and email
...
router.get("/my", authMiddleware, async (req, res) => {
  try {
    ...
    .populate("deliveryAgentId", "name email role")  // populate agent info
      .sort({ createdAt: -1 });
    res.json({ success: true, pickups });
  } catch (error) {
